...
class ChapterBand(Flowable):
    ...
    def wrap(self, availWidth, availHeight):
        ...
        for word in words:
            ...
            if not current or stringWidth(candidate, FONTS["bold"], 15) <= max_width:
                current = candidate
            else:
...
def as_flowables(item) -> list:
    return item if isinstance(item, list) else [item]


def trim_trailing_section_rule(story: list) -> None:
    removed_spacers = []
    while story and isinstance(story[-1], Spacer):
        removed_spacers.append(story.pop())
    if story and isinstance(story[-1], SectionRule):
        story.pop()
    else:
        story.extend(reversed(removed_spacers))
...
def make_table(block: TableBlock):
    ...
    table.setStyle(TableStyle(commands))
    flowables = [Spacer(1, 3), table, Spacer(1, 10)]
    if len(rows) <= 9:
...
def render_profile_lines(lines: list[str]):
    ...
    return KeepTogether([Spacer(1, 3), header, cards, Spacer(1, 11)])
...
def parse_body(lines: list[str]) -> list:
    ...
    def flush_lists() -> None:
        ...
        if list_items:
            story.extend(as_flowables(make_list(list_items, ordered=False)))
            list_items = []
        if ordered_items:
            story.extend(as_flowables(make_list(ordered_items, ordered=True)))
            ordered_items = []
    ...
    while index < len(lines):
        ...
        if line.startswith("|") and "|" in line[1:]:
            ...
            if len(rows) >= 2 and is_table_separator(rows[1]):
                ...
                table_rows = rows
            story.extend(as_flowables(make_table(TableBlock(table_rows, aligns))))
            continue
        ...
        if heading_match:
            ...
            if level == 1:
                trim_trailing_section_rule(story)
                if first_h1:
                    first_h1 = False
                else:
                ...
                if level == 2 and text == "Gründerprofil":
                    ...
                    while lookahead < len(lines):
                        ...
                        lookahead += 1
                    story.extend(as_flowables(render_profile_lines(profile_lines)))
                    index = lookahead
                    continue
            index += 1
            continue
