...
.toc-row {{ display: grid; grid-template-columns: 20mm 1fr 12mm; align-items: center; padding: 4.4pt 0; border-bottom: .45pt solid transparent; }}
...
.profile-block {{ margin: 3pt 0 11pt; break-inside: avoid; page-break-inside: avoid; }}
.profile-header {{
  margin-top: 3pt;
  padding: 6pt 8pt;
  border-bottom: 2pt solid var(--gold);
...
function cleanupDanglingTails(root) {{
  const badTail = (node) => node?.matches?.("h2,h3,h4,.section-rule,.profile-header");
